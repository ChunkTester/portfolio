let sketch = function(p) {
	...
	function flow() {
		...
		hsb[0] = p.map(p.mouseX, 0, p.width, 0, 360);	//hue
		hsb[1] = p.map(p.mouseY, 0, p.height, 20, 100);	//sat
		hsb[2] = p.map((10 * p.sin(p.millis() / 500)), 0, p.height, 50, 100);	//bri
...
waitForElm('.p5Canvas').then((elm) => {
	console.log('Element is ready');
    
	mouseTarget = document.getElementById("defaultCanvas0");
	
	if (mouseTarget.getContext) {
		var ctx = mouseTarget.getContext('2d');
		ctx.restore();
